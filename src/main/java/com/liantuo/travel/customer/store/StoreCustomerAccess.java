...
public class StoreCustomerAccess extends BaseFunction {
	/**
	 * 
	 */
	private static final long serialVersionUID = -134360533028883430L;
	private static final String FIELDS = "app_name,req_time,num,req_date";
	private static final String TABLE_NAME = "travel_customer_access";
	...
	private void executeBatch(Queue<TridentTuple> tupleQueue, PreparedStatement ps) throws SQLException {
		for (int i = 0; i < tupleQueue.size(); i++) {
			...
			ps.setString(2, tuple.getStringByField("reqTime"));
			ps.setLong(3, tuple.getLongByField("num"));
			ps.setString(4, tuple.getStringByField("reqTime").substring(0, 10));
			ps.addBatch();
		}

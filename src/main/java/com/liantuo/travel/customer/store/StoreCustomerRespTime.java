...
public class StoreCustomerRespTime extends BaseFunction {
	/**
	 * 
	 */
	private static final long serialVersionUID = -134360533028883430L;
	private static final String FIELDS = "app_name,server_ip,req_time,num,req_date,status,resp_time";
	private static final String TABLE_NAME = "customer_resp_time";
	...
	private void executeBatch(Queue<TridentTuple> tupleQueue, PreparedStatement ps) throws SQLException {
		for (int i = 0; i < tupleQueue.size(); i++) {
			...
			ps.setString(6, tuple.getStringByField("status"));
			ps.setDouble(7, tuple.getDoubleByField("respTimeSum"));
			ps.addBatch();
		}
